...
public class RepeatedDataStorageAction extends Action
{
    private final boolean            isWordSize;
    private final List<Action>       count;
    private final List<List<Action>> expressions;

    public RepeatedDataStorageAction(final FileLocation location, final boolean isWordSize,
            final List<Action> count, final List<List<Action>> expressions)
    {
        super(location);
    ...
    {
        long todo = assembler.evalExpression(this.count);
        if (todo < 0)
        {
            this.error("Illegal REP count " + todo);
        }
        while (todo > 0)
        {
